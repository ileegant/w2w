...
export default function TrandingNow() {
  ...
  return (
    <div className="flex gap-4 p-4 w-full overflow-x-auto scrollbar-hide snap-x snap-mandatory bg-neutral-800">
      {movies?.map((movie) => (
        <div
          key={movie.id}
          className="group flex flex-col bg-neutral-900/50 border border-neutral-800 text-neutral-50 rounded-2xl p-3 gap-3 shrink-0 snap-start hover:bg-neutral-900 transition-all duration-300 hover:-translate-y-2 w-50"
        >
          <div className="relative aspect-2/3 overflow-hidden rounded-xl shadow-lg">
            ...
            />
            <div className="flex items-center gap-1 absolute top-2 right-2 bg-black/60 backdrop-blur-md px-2 py-1 rounded-lg text-sm font-bold text-amber-400 border border-white/10 shadow-xl">
